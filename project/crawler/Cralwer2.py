...
def check_if_maven(full_name):
    ...
    try:
        ...
        print(f"[DEBUG] Remaining quota: {remaining} for {full_name}")
        ...
        print(f"[ERROR] check_if_maven failed for {full_name}: {e}")
...
def crawl_java_maven_projects():
    seen_ids = set()
    total_sent = 0
    all_projects = []  # for optional file saving
    ...
    if SAVE_TO_FILE:
        ...
        print(f"[INFO] Saved {len(all_projects)} projects to {OUTPUT_JSON}")
